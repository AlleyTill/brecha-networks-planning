...
| Tailwind integration | `django-tailwind-cli` (standalone Tailwind binary, no Node/npm) |
...
- `django-tailwind` (timonweb): functional but Node/npm-dependent. On Windows specifically, Python's subprocess calls cannot find `npm` because it's `npm.cmd` (timonweb/django-tailwind#91, open). Plus `node_modules` brings real MAX_PATH risk on Windows. **Swapped to `django-tailwind-cli`** (django-commons project) which downloads the standalone Tailwind binary directly — no Node, no npm, no `node_modules`. Eliminates the entire Windows Node toolchain from the build. Decision locked 2026-05-16.
...
- **May 16, 2026 (Tailwind integration swap — Windows-driven):** Swapped `django-tailwind` → `django-tailwind-cli`. Pre-build research (Reddit, GitHub issues, dev.to, discuss.python.org, last 12-18 months) surfaced concrete Windows pain with the Node-based `django-tailwind`: Python can't resolve `npm` because it's `npm.cmd` (timonweb/django-tailwind#91, still open), and `node_modules` carries a real MAX_PATH risk on Windows. `django-tailwind-cli` (django-commons) uses the standalone Tailwind binary directly — no Node, no npm, no `node_modules`. Updated the locked package table and maintenance-verification notes. Companion operational notes (not locks) recorded in `build-plan.md` Week 1: enable Windows long paths (`LongPathsEnabled` + `git config --global core.longpaths true`), Defender exclusions for `C:\dev\` and `python.exe`, pin `django-allauth` to a specific 65.x release.
